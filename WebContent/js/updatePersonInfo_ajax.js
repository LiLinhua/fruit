...
{
	createXMLHttp();
	xmlHttp.open("POST","Servlet_updatePersonInfo_ajax?cus_name="+cus_name);
...
function checkCusNameCallback()
{
	if(xmlHttp.readyState == 4)
	{
		if(xmlHttp.status == 200)
		{
			...
			{
				document.getElementById("nameError").innerHTML = "<font color='red'>用户名被使用</font>";
...
{
	createXMLHttp();
	xmlHttp.open("POST","Servlet_updatePersonInfo_ajax?cus_phone="+cus_phone);
...
function checkCusPhoneCallback()
{
	if(xmlHttp.readyState == 4)
	{
		if(xmlHttp.status == 200)
		{
			...
			{
				document.getElementById("phoneError").innerHTML = "<font color='red'>手机号码被使用</font>";
...
{
	createXMLHttp();
	xmlHttp.open("POST","Servlet_updatePersonInfo_ajax?cus_mail="+cus_mail);
...
function checkCusMailCallback()
{
	if(xmlHttp.readyState == 4)
	{
		if(xmlHttp.status == 200)
		{
			...
			{
				document.getElementById("mailError").innerHTML = "<font color='red'>邮箱被使用</font>";
