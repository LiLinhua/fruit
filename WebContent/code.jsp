...
<%
  ...
  for (int i = 0; i < 4; i++)
  {
	  ...
	  g.setColor(getRandColor(random, 100, 200));
	  //设置字体
	  g.setFont(new Font(fontTypes[random.nextInt(fontTypesLength)], Font.PLAIN, 20 + random.nextInt(6)));
 	  //将字体画到图片上
	  g.drawString(rand, 24 * i + 5 + random.nextInt(25), 24);
  }
  //将认证码存入session
 // HttpSession ses=request.getSession(true);
//  ses.setAttribute("checkCode",sRand); 
//  ses.setMaxInactiveInterval(30 * 60);
  session.setAttribute("checkCode", sRand);
  System.out.println((String)session.getAttribute("checkCode")+"11111111111111111");
  g.dispose();
  //输出图像到页面
  out.clear();
